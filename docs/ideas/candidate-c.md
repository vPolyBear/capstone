...
Today they                    can use period tracking apps just as Clue or try to track manually
which falls short because     each app fails to properly account for everything each individual women goes through such as sleep disruptions, a missed birth control pill, health issues, and so on which all individually affect the cycle and tracking manually is extremely hard
